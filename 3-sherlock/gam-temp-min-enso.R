...
# @File         : gam-temp-min-enso.R
# @Description  : DLNM-GAM for minimum temperature stratified by ENSO period
...
saveRDS(base_result, paste0(results_path, "base-temp-min-enso-normal.RDS"))
...
saveRDS(boot_results, file = paste0(results_path, "bs-temp-min-enso-normal.RDS"))
...
base_result <- compute_marginal_incidence(model, analysis_data, pred_seq, predictor, lag_value)
saveRDS(base_result, file = paste0(results_path, "base-temp-min-enso-el-nino.RDS"))

# Perform bootstrap for El Niño data
cat("Starting bootstrap for El Niño data...\n")
boot_results <- bootstrap_incidence_parallel(analysis_data, pred_seq, predictor, lag_value, covariate, cores_per_worker, n_boot = 1000)
saveRDS(boot_results, file = paste0(results_path, "bs-temp-min-enso-el-nino.RDS"))
...
base_result <- compute_marginal_incidence(model, analysis_data, pred_seq, predictor, lag_value)
saveRDS(base_result, file = paste0(results_path, "base-temp-min-enso-la-nina.RDS"))

# Perform bootstrap for La Niña data
cat("Starting bootstrap for La Niña data...\n")
boot_results <- bootstrap_incidence_parallel(analysis_data, pred_seq, predictor, lag_value, covariate, cores_per_worker, n_boot = 1000)
saveRDS(boot_results, file = paste0(results_path, "bs-temp-min-enso-la-nina.RDS"))
